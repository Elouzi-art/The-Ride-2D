...
GameMap2::GameMap2(SDL_Window* win, SDL_Renderer* rend)
    : window(win), renderer(rend), running(true), level(0), timeElapsed(0), startTime(SDL_GetTicks())
{
    ...
    Mix_VolumeChunk(jumpSound, MIX_MAX_VOLUME / 8);  // pour le 25% volume
    ...
    SDL_Surface* surface = IMG_Load("res/player2.png"); 
if (!surface)
{
...
bool GameMap2::checkCollision(const SDL_Rect &a, const SDL_Rect &b)
{
    return SDL_HasIntersection(&a, &b);
}
// Methode de generation des obstacles en se basant sur la fenetre donne
void GameMap2::generateObstacle(M12_Obstacle &ob)
{
    const int spacingMin = 300;
    const int spacingMax = 500;
