...
GameMode11::GameMode11(SDL_Window* win, SDL_Renderer* rend)
    : window(win),renderer(rend), running(true), level(0), timeElapsed(0), startTime(SDL_GetTicks())
{
    ...
    SDL_Surface* surface = IMG_Load("res/player1.png"); // Change to your correct path
if (!surface)
{
...
for (const std::string& path : obstaclePaths) {
    ...
    font = TTF_OpenFont("res/ARIALBLACKITALIC.TTF", 24); // Utiliser une police existante (ici "arial.ttf")
...
bool GameMode11::checkCollision(const SDL_Rect &a, const SDL_Rect &b)
{
    return SDL_HasIntersection(&a, &b);
    }

void GameMode11::generateObstacle(Obstacle &ob)
{
    const int spacingMin = 300;
    const int spacingMax = 500;
