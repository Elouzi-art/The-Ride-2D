...
GameMode12::GameMode12(SDL_Window* win, SDL_Renderer* rend)
    : window(win),renderer(rend), running(true), level(0), timeElapsed(0), startTime(SDL_GetTicks())
{
    ...
    TTF_Init();                                  // Initialiser la bibliothèque SDL_ttf
    ...
    SDL_Surface* bgSurface = IMG_Load("res/map2.jpg");
    if (!bgSurface) {
    ...
    SDL_SetHint(SDL_HINT_RENDER_SCALE_QUALITY, "0"); // "0" pour Nearest pixel sampling
    SDL_Surface* surface = IMG_Load("res/player2.png"); // Change to your correct path
if (!surface)
{
    std::cerr << "Failed to load player image: " << SDL_GetError() << std::endl;
}
else
{
...
for (const std::string& path : obstaclePaths) {
    ...
    // Charger la police pour afficher le temps et le level
    font = TTF_OpenFont("res/ARIALBLACKITALIC.TTF", 24);
    infoFont = TTF_OpenFont("res/ARIALBLACKITALIC.TTF", 16); // Utiliser une police existante (ici "arial.ttf")
    if (!font)
    {
    ...
    while (SDL_PollEvent(&event))
    {
        ...
        else if (event.type == SDL_KEYDOWN)
        {
            ...
            else if (event.key.keysym.sym == SDLK_SPACE && jumpCount < 2)
            {
                ...
                gameMusic = Mix_LoadMUS("audio/game2_background.wav");
                if (!gameMusic) {
...
void GameMode12::update()
{
    ...
    timeElapsed = (SDL_GetTicks() - startTime) / 1000; // en secondes
    // Variable pour garder la trace du moment où 20 secondes se sont écoulées
    // Si 20 secondes sont écoulées, accélérer la vitesse
    if (timeElapsed > 10 && timeElapsed / 10 > lastTimeCheckpoint)
    {
        velocityX = 6; // Accélérer la vitesse des obstacles
        // Incrémenter le level chaque fois que 20 secondes sont passées
        level++;

        // Mettre à jour le checkpoint
        lastTimeCheckpoint = timeElapsed / 10;
    }
    for (auto &ob : obstacles) {
        ob.rect.x -= 5;
...
bool GameMode12::checkCollision(const SDL_Rect &a, const SDL_Rect &b)
{
    return SDL_HasIntersection(&a, &b);
    }

void GameMode12::generateObstacle(M22_Obstacle &ob)
{
    const int spacingMin = 300;
    const int spacingMax = 500;
